...
export class EngineBuilder {
    ...
    private getGarbageCollectorCDF(): CDFItem[] {
        ...
        return files.map(file => ({
            ...
            arguments: this.gateway.compiler.buildCDFArray(path.join(this.objFolder, "bdwgc", path.basename(file) + ".o"), ["-I", path.join(ENGINE_BASE, "bdwgc"), "-I", path.join(ENGINE_BASE, "bdwgc", "private"), "-DENABLE_THREADS=0", "-DGC_DISABLE_INCREMENTAL=1", "-DNO_INCREMENTAL=1", "-DNO_CLOCK=1", "-ULARGE_CONFIG", "-DSTATIC_LINK", "-DNO_GETCONTEXT", "-Wno-macro-redefined"]),
            file: file
        }));
    }
    ...
    private compileGarbageCollector(): string {
        ...
        const objectFiles: string[] = this.compileCFiles(inputFiles, base, ["-I", path.join(ENGINE_BASE, "bdwgc"), "-I", path.join(ENGINE_BASE, "bdwgc", "private"), "-DENABLE_THREADS=0", "-DGC_DISABLE_INCREMENTAL=1", "-DNO_INCREMENTAL=1", "-DNO_CLOCK=1", "-ULARGE_CONFIG", "-DSTATIC_LINK", "-DNO_GETCONTEXT ", "-Wno-macro-redefined"]);
